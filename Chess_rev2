...
// Changed from NEO_GRBW to NEO_GRB since there's no white channel
...
// Button State Tracking for stable sensor reading
...
// Updated colors for GRB LEDs (no white channel)
// For white, use equal values of R, G, B
const uint32_t PROMOTION_COLOR = strip.Color(255, 215, 0); // Gold color for promotion
const uint32_t WHITE_COLOR = strip.Color(255, 255, 255); // White using RGB
const uint32_t RED_COLOR = strip.Color(255, 0, 0); // Red for captures
const uint32_t DIM_WHITE = strip.Color(100, 100, 100); // Dimmer white
...
void setup() {
  ...
  for (int c = 0; c < NUM_COLS; c++) {
    pinMode(colPins[c], INPUT_PULLUP); // Changed to INPUT_PULLUP for stable readings
  }
...
void loop() {
  ...
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      if (sensorState[row][col] != sensorPrev[row][col]) {
        matrixChanged = true;
      }
    }
  }
  ...
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      if (sensorPrev[row][col] && !sensorState[row][col]) {
        ...
        strip.setPixelColor(currentPixelIndex, DIM_WHITE); // Dim white for source square
        ...
        for (int i = 0; i < moveCount; i++) {
          ...
          if (board[r][c] == ' ') {
            strip.setPixelColor(movePixelIndex, DIM_WHITE); // Dim white for moves
          } else {
            strip.setPixelColor(movePixelIndex, RED_COLOR); // Red for captures
          }
        }
        strip.show();
        ...
        while (!piecePlaced) {
          ...
          for (int r2 = 0; r2 < 8; r2++) {
            for (int c2 = 0; c2 < 8; c2++) {
              ...
              if (board[r2][c2] != ' ' && !sensorState[r2][c2] && sensorPrev[r2][c2]) {
                ...
                strip.setPixelColor(capturePixelIndex, RED_COLOR);
                strip.show();
          ...
          // Update sensor previous state to avoid false readings
          for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
        ...
        if (targetRow == row && targetCol == col) {
          ...
          strip.setPixelColor(currentPixelIndex, WHITE_COLOR);
          strip.show();
          delay(200);
          strip.setPixelColor(currentPixelIndex, DIM_WHITE);
          strip.show();
        ...
        if (legalMove) {
          ...
          for (int blink = 0; blink < 2; blink++) {
            strip.setPixelColor(newPixelIndex, WHITE_COLOR);
            strip.show();
            delay(200);
            strip.setPixelColor(newPixelIndex, DIM_WHITE);
            strip.show();
            delay(200);
          }
        } else {
  ...
  delay(50);  // Reduced delay for more responsive readings
}
...
void readSensors(){
  bool tempSensorState[8][8] = {0}; // Use temporary array for stable reading
  
  for (int row = 0; row < 8; row++){
    loadShiftRegister(rowPatterns[row]);
    delayMicroseconds(200); // Increased delay for signal stabilization
    
    for (int col = 0; col < NUM_COLS; col++){
      int sensorVal = digitalRead(colPins[col]);
      tempSensorState[row][col] = (sensorVal == LOW);
    }
  }
  
  // Copy from temp array to actual sensor state
  for (int row = 0; row < 8; row++){
    for (int col = 0; col < NUM_COLS; col++){
      sensorState[row][col] = tempSensorState[row][col];
    }
  }
...
void updateSetupDisplay(){
  for (int row = 0; row < 8; row++){
    for (int col = 0; col < 8; col++){
      ...
      if(initialBoard[row][col] != ' ' && sensorState[row][col]){
         strip.setPixelColor(pixelIndex, WHITE_COLOR); // White color
      } else {
...
// New function for updating LEDs (only used if we implement the full state tracking system)
void updateLEDs() {
  // Clear all LEDs
  for (int i = 0; i < LED_COUNT; i++) {
    strip.setPixelColor(i, 0);
  ...
  // Update based on current board state
  for (int row = 0; row < 8; row++) {
    for (int col = 0; col < 8; col++) {
      if (board[row][col] != ' ') {
        int pixelIndex = col * NUM_COLS + (7 - row);
        
        // Use white for all pieces
        strip.setPixelColor(pixelIndex, DIM_WHITE); // Dim white
      }
    }
  }
